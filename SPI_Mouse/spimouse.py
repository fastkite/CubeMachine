...
ncs = 18 # pin for chip select / slave select / enable
motion = 23 # i think this is the mouse telling me motion has occured
spi = None
...
def setup():
    ...
    spi.max_speed_hz = 4000 # max 4 Mhz
    ...
    performStartup()
    print("ADNS9800testPolling")
    dispRegisters()

    time.sleep(5) # delay(5000)
    #initComplete = True
...
def dispRegisters():
    ...
    adns_com_begin() #digitalWrite(ncs,LOW);

    for i in range(4):
        spi.writebytes([oreg[i]])
        time.sleep(0.001)  # delay(1)

        regres = spi.readbytes(1)[0] # just get 1 byte, and extract it from the list which is returned by using [0]

        print ("REG_{} (0x{:02x}) = 0x{:02x} / {} ".format(oregname[i],oreg[i],regres,regres))
        # print (oregname[i])
        # print(hex(oreg[i]))
        # print (regres)
        # print(hex(regres))
        time.sleep(0.001)  # delay(1)
    adns_com_end()  # digitalWrite(ncs,HIGH);
"""
...
def adns_write_reg(reg_addr, data):

    adns_com_begin()
    #send adress of the register, with MSBit = 1 to indicate it's a write
    new_addr = reg_addr | 0x80
    print ("0x{:02x} | 0x80 = 0x{:02x}".format(reg_addr,new_addr))
    spi.writebytes([new_addr])
    spi.writebytes([data])
    time.sleep(0.000020) #tSCLK-NCS for write operation (20 microseconds)
    adns_com_end()
    time.sleep(0.000100)# tSWW/tSWR (=120us) minus tSCLK-NCS. Could be shortened, but is looks like a safe lower bound (100 microseconds)

"""

void adns_write_reg(byteREG_addr, byte data){
  adns_com_begin();
...
def adns_read_reg(reg_addr):
    adns_com_begin()
    spi.writebytes([reg_addr & 0x7f])
    time.sleep(0.000100)# tSRAD
    data = spi.readbytes(1) # read one byte
    time.sleep(0.000001)# tSCLK-NCS for read operation is 120ns
    adns_com_end()
    time.sleep(0.000019)  # tSCLK-NCS for read operation is 120ns
    return data[0]
...
def adns_upload_firmware():
    ...
    adns_com_begin()
    spi.writebytes([REG_SROM_Load_Burst | 0x80]) # write burst destination adress
    time.sleep(0.000015)  # (15 microseconds)

    # send all bytes of the firmware
    for b in firmware_data:
        spi.writebytes([b])
        time.sleep(0.000015)  # (15 microseconds)
    adns_com_end()
...
def performStartup():
    ...
    print("laser_ctrl0: 0x{:02x}".format(laser_ctrl0))
    laser_ctrl0  = laser_ctrl0 & 0xf0
    print("laser_ctrl0 & 0xf0: 0x{:02x}".format(laser_ctrl0))
...
def UpdatePointer():
    adns_com_begin() #digitalWrite(ncs,LOW);
    x = adns_read_reg(REG_Delta_X_L)
    y = adns_read_reg(REG_Delta_Y_L)
    adns_com_end()#digitalWrite(ncs,HIGH);
    return x,y
